...
class XboxAuthApi {
     ...
     * Returns an XboxAuthResponse containing the Xbox User Token and user hash.
     */
    ...
    /**
     * Obtains an XSTS token scoped to Minecraft services.
     * Throws XboxAuthException for known Xbox Live error codes.
     */
    fun obtainXstsToken(xboxUserToken: String): XboxAuthResponse {
        val requestBody = mapOf(
            "Properties" to mapOf(
                "SandboxId" to "RETAIL",
                "UserTokens" to listOf(xboxUserToken)
            ),
        ...
        if (response.statusCode() != 200) {
            ...
            if (response.statusCode() == 401) {
                try {
                    val error = objectMapper.readValue(response.body(), XstsErrorResponse::class.java)
                    throw XboxAuthException(error.xErr, error.redirect)
                } catch (e: JsonProcessingException) {
                    throw IOException(
                        "XSTS authentication failed with status 401, body: ${response.body()}", e
                    )
                }
            }
    ...
    data class XboxAuthResponse @JsonCreator constructor(
        ...
        val gamertag: String? get() = displayClaims?.xui?.firstOrNull()?.gtg
        val xboxUserId: String? get() = displayClaims?.xui?.firstOrNull()?.xid
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    data class DisplayClaims @JsonCreator constructor(
        @JsonProperty("xui") val xui: List<XuiClaim>?
    )

    @JsonIgnoreProperties(ignoreUnknown = true)
    data class XuiClaim @JsonCreator constructor(
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    data class XstsErrorResponse @JsonCreator constructor(
    ...
    /**
     * Exception for known Xbox Live authentication error codes.
     * Includes redirect URL in the message where provided (e.g. child-account setup links).
     */
        ...
        companion object {
            fun buildMessage(errorCode: Long, redirectUrl: String?): String {
                val base = when (errorCode) {
                    ...
                    2148916233L -> "This Microsoft account has no Xbox account. " +
                        "Please create one at xbox.com/live"
                    2148916235L -> "Xbox Live is not available in your country."
                    2148916236L, 2148916237L -> "This account requires adult verification (South Korea)."
